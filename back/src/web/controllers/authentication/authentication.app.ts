...
export class AuthenticationApp {
	static log = getLogger.controller(AuthenticationApp);
	...
	async createPermanentAppToken(@PathParams("app") app: string, @Req() { auth }: Request) {
		AuthenticationApp.log.warn(`${auth!.username} is creating a permanent token for the app ${app}`);
